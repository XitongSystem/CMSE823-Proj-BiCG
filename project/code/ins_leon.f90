module type_defs
  ! This module defines single, double and quadruple precision numbers.
...
module problem_setup
  ! This module defines the problem, length, height and viscosity
  ...
  real(dp), parameter :: k = 0.005_dp      ! Timestep
...
module arrs
  ...
  ! Velocity in the x-direction, y-direction and pressure (curent and previous timestep)
  real(dp), allocatable, dimension(:,:) :: u,v,p
  real(dp), allocatable, dimension(:,:) :: uold,vold,pold
  ! The equations are on the form w_t = lew(u,v,p) + liw(u,v,p), where w = u,v,p
  real(dp), allocatable, dimension(:,:) :: leu,lev,liu,liv
  real(dp), allocatable, dimension(:,:) :: leuold,levold,liuold,livold
  ! Boundary conditions for u and v along top, bottom, left and right boundaries
  real(dp), allocatable, dimension(:,:) :: gux,guy,gvx,gvy,pbx,pby
end module arrs

module matrices
  ! This module holds matrices such that solving A w^{n+1} = b, gives the solution at the
  ! next timestep. These matrices are stored as dense matrices even though they are sparse.
  ! This sturage and direct solve by Gauss E. is the main bottleneck of the code
  ! and your assignment is to remove it.
  use type_defs
  implicit none
  ...
  real(dp), allocatable, dimension(:) :: uvec2,vvec2 !!!!!!!!!
  real(dp), allocatable, dimension(:) :: pbvecbig,pvecbig
end module matrices

module afuns
  ! This modle holds "matrix free" versions of the matrices (or part of them) above.
...
contains
  ...
  subroutine apply_pressure_laplacian(lp,p,nx,ny,hx,hy)
    ...
    do j=1,my
       do i=1,mx
          ! convert 2D index to a linear "vector" index
          k = i+(j-1)*mx
          kp = k+1
          km = k-1
          kb = k-mx
...
! Module for Dirichlet BiCG
module iterative_solver_D
  use type_defs
  implicit none
  real(dp), allocatable, dimension(:) :: r_isd,x_isd,Ar_isd,pp
...
contains
  ...
  subroutine allocate_isd(n)
    implicit none
    integer, intent(in) :: n
    allocate(r_t(1,n),r_isd(n),x_isd(n),x_t(1,n),Ar_isd(n),pp(n),p_t(1,n))
  end subroutine allocate_isd

  subroutine deallocate_isd
    implicit none
    !deallocate(r_isd,x_isd,Ar_isd)
    deallocate(r_t, r_isd,x_isd,x_t,Ar_isd,pp,p_t)
  end subroutine deallocate_isd
  
  subroutine bicg_d(x,b,n,nx,ny,hx,hy,nu,k)
    use type_defs
    use afuns
    implicit none
    integer, intent(in) :: n
    ...
    do while ((res_norm2/res_norm20 .gt. TOL_ISD**2) &
       ...
       call apply_velocity_laplacian(Ar_isd,pp,nx,ny,hx,hy)
       Ar_isd = pp/k - nu*Ar_isd/2.0_dp
       
       beta_d = sum(r_t(1,:)*r_isd)
       alpha = beta_d / sum(p_t(1,:)*Ar_isd)
       x_isd = x_isd + alpha*pp
       x_t   = x_t   + alpha*p_t
       r_isd = r_isd - alpha*Ar_isd 
       
       !call apply_1D_laplacian_D(Ar_isd,p_t(1,:),n)
       call apply_velocity_laplacian(Ar_isd,p_t(1,:),nx,ny,hx,hy)
       Ar_isd = p_t(1,:)/k - nu*Ar_isd/2.0_dp
...
program ins
  ...
  ! Leading dimensions of various matrices
  ...
  if (use_dense_uv) then
     ...
     ! setup uv laplacian + timestepping modification along diagonal.
     uvec = 0.0_dp
     do i = 1,(nx-1)*(ny-1)
        uvec(i) = 1.0_dp
     ...
     ! Do necessary precomputations / allocations here for BiCG
  ...
  if (use_dense_p) then
     ...
     ! Factor
     ! N.B. once factored LapPBig and LapUV are overwritten by the L*U factors  
  ...
  do nt = 1,Nsteps
     ...
     if (use_dense_uv) then
        ...
        !!!! Solve velocity problem with BiCG !!!!
        call allocate_isd(sys_size_uv)
        call set_tol_isd(tol)
        call bicg_d(uvec,uvec,sys_size_uv,nx,ny,hx,hy,nu,k)
        call bicg_d(vvec,vvec,sys_size_uv,nx,ny,hx,hy,nu,k)
        ! Swap long vector into 2D array
        do j = 1,ny-1
           do i = 1,nx-1
              u(i,j) = uvec(i+(j-1)*(nx-1))
