module type_defs
 ...
 module iterative_solver_D
   use type_defs
   implicit none
   real(dp), allocatable, dimension(:) :: r_isd,x_isd,Ar_isd,p_d
   real(dp), allocatable, dimension(:,:) :: r_t, p_d_t, x_t
   real(dp) :: TOL_ISD
 contains
   ...
   subroutine allocate_isd(n)
     implicit none
     integer, intent(in) :: n
     allocate(r_t(1,n),r_isd(n),x_isd(n),x_t(1,n),Ar_isd(n),p_d(n),p_d_t(1,n))
   end subroutine allocate_isd
 
   subroutine deallocate_isd
     implicit none
     deallocate(r_t,r_isd,x_isd,x_t,Ar_isd,p_d,p_d_t)
   end subroutine deallocate_isd
   
   subroutine bicg_d(x,b,A,n)
     use type_defs
     use afuns
     implicit none
     integer, intent(in) :: n
...
module iterative_solver_N
   use type_defs
   implicit none
   real(dp), allocatable, dimension(:) :: r_isn,x_isn,Ar_isn,p_n
   real(dp), allocatable, dimension(:,:) :: r_t, p_n_t, x_t
   real(dp) :: TOL_ISN
 contains
   ...
   subroutine allocate_isn(n)
     implicit none
     integer, intent(in) :: n
     allocate(r_isn(n),x_isn(n),Ar_isn(n),r_t(1,n),x_t(1,n),p_n(n),p_n_t(1,n))
 ...
 program ins
   ...
   do nt = 1,Nsteps
      ...
      if (use_dense_uv) then
         ...
         call bicg_d(uvec,uvec,LapUV,sys_size_uv)
         call bicg_d(vvec,vvec,LapUV,sys_size_uv)
         ! Swap long vector into 2D array
         do j = 1,ny-1
            do i = 1,nx-1
               u(i,j) = uvec(i+(j-1)*(nx-1))
